...
def intro(request):
    return render(request, 'sub_01.html')


def produce(request):
    return render(request, 'sub_02.html')

def business_01(request):
    return render(request, 'business_01.html')

def business_02(request):
    return render(request, 'business_02.html')

def business_03(request):
    return render(request, 'business_03.html')

def business_04(request):
    return render(request, 'business_04.html')

def business_05(request):
    return render(request, 'business_05.html')

def river_intro(request):
    return render(request, 'river_intro.html')

def predict_set(request, model):
    return render(request, 'predict_set.html', {'model': model})
    ...
    if request.is_ajax():
        ...
        df_sample = pd.DataFrame(
            {'측정망': target_list,
             ...
             '경도': x_list,
             '집수구역 ID': cat_id_list,
             '분할구역 ID': cat_did_list,
             '리치 ID': rch_id_list,
             '리치 DID': rch_did_list,
             '노드 ID': node_id_list,
             '노드 DID': node_did_list
             })
        # HTML로 변환하기
...
def color_list(key, data_list):
    ...
    if key == 'toc':
        for i in data_list:
            if i <= 2:
                color.append('rgba(13,238,98,0.95)')
            elif 2 < i <= 3:
                color.append('rgba(153,235,127,0.95)')
            elif 3 < i <= 4:
                color.append('rgba(223,238,153,0.95)')
            elif 4 < i <= 5:
                color.append('rgba(235,223,190,0.95)')
            elif 5 < i <= 6:
                color.append('rgba(235,172,133,0.95)')
            elif 6 < i <= 8:
                color.append('rgba(235,80,51,0.95)')
            elif 8 < i:
                color.append('rgba(235,0,20,0.95)')
    elif key == 'chl':
        for i in data_list:
            if i <= 5:
                color.append('rgba(13,238,98,0.95)')
