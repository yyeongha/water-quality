...
def gain (data_x, gain_parameters):
  ...
  ds = tf.data.Dataset.from_generator(
      lambda: train_generator.__iter__(),
      output_types=({
        'model_input_x': tf.float32,
        'model_input_m': tf.float32,
        'model_input_h': tf.float32,
      }),
      output_shapes=( {
        'model_input_x': [batch_size, train_generator.dim],
        'model_input_m': [batch_size, train_generator.dim],
        'model_input_h': [batch_size, train_generator.dim],
      }
      )
  )
  ...
  #for it in range(iterations):    
    ...
    tock = time.time()
    print('get tensor time = ', (tock-tick)*1000)
