...
class MissData(object):
    ...
    def __init__(self, load_dir=False):
        if load_dir:
            self.missarr = np.load('./save/miss.npy')
            self.idxarr = np.load('./save/idx.npy')


    def make_missdata(self, data_x, missrate=None):

        # backup columns
        backup_col = data_x.columns
        ...
        # numpy to dataframe
        normalization_df = pd.DataFrame(data, columns=backup_col)
